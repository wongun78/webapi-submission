...
public class SecurityUtil {
    ...
    public String createAccessToken(String email, AuthResponse dto) {
        UserResponse userToken = new UserResponse();
        userToken.setId(dto.getUser().getId());
        userToken.setEmail(dto.getUser().getEmail());
        userToken.setUsername(dto.getUser().getUsername());
        userToken.setRoles(dto.getUser().getRoles());

        Instant now = Instant.now();
        Instant validity = now.plus(this.accessTokenExpiration, ChronoUnit.SECONDS);

        // Use actual user roles from database
        List<String> roles = new ArrayList<>(dto.getUser().getRoles());
